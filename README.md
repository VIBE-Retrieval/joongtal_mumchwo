...
```
[학생] 오늘의 설문 응답 (성취도 / 적응도 / 인간관계, 각 1~5점)
    │
    ▼
[ML 모델] 최근 7일치 응답을 평균 내어 위험도 점수 계산
    │   (4개 feature 입력 → 4-7-1 신경망 → risk_score 0~1)
    │
    ▼
[LLM / Claude] 위험도 점수와 등급을 받아 "왜 위험한가"를 자연어로 해석
    │   state_summary / risk_reason / risk_type
    │
    ▼
[Agent] LLM 해석 + 과거 개입 이력을 종합해 행동 결정 및 실행
    │   NONE / ENCOURAGE_MESSAGE / ALERT_MENTOR / REQUEST_MEETING / EMERGENCY
    │
    ▼
[멘토] 케어 완료 후 피드백 입력 → DB 누적 → Agent 보정
```

**각 계층의 역할은 엄격히 분리됩니다.**

- **ML** — 숫자만 계산합니다. 해석하지 않습니다.
- **LLM** — ML 결과를 사람의 언어로 번역합니다. 계산하지 않습니다.
- **Agent** — LLM 해석과 과거 이력을 보고 행동을 결정합니다. 직접 계산하거나 해석하지 않습니다.
...
1. 학생이 오늘의 설문 응답 (성취도 / 적응도 / 인간관계, 각 1~5점)
        ↓
2. 과정 ML이 최근 7일치 평균으로 risk_score 계산
   (4-7-1 신경망 → risk_score 0~1, risk_level, risk_trend 출력)
        ↓
3. LLM(Claude)이 위험도를 자연어로 해석
   → "최근 성취도와 적응도가 연속 하락 중입니다"
   → "학습 이해 부족이 누적되어 자신감이 저하된 것으로 보입니다"
        ↓
4. Agent가 LLM 해석 + 과거 개입 이력을 종합해 행동 결정 및 실행
        ↓
5. 멘토가 케어 완료 후 피드백 입력
   (recovered / false_alarm / 효과 없음)
        ↓
6. 피드백 DB 누적 → 다음 판단 시 Agent 보정
```
...
├── DB/              # 스키마 정의, 마이그레이션, 시드 데이터
├── AGENTS/          # 도메인별 AI 에이전트 행동 지침
...
- D7~D10: 점수 연속 하락. Agent `ENCOURAGE_MESSAGE` 발송.
- D11: HIGH 2일 연속 → Agent `ALERT_MENTOR` 실행. 멘토 직접 개입.
- D12~D14: 점수 회복, LOW 복귀. 멘토 피드백 "recovered" → Agent가 ALERT_MENTOR 효과 학습.

**이서연 — 고위험 지속 (EMERGENCY 대기 중)**
- D1~D4: 빠른 하락. Agent `ENCOURAGE_MESSAGE` → 효과 없음.
- D5~D7: HIGH 3일 연속 → `ALERT_MENTOR`. 멘토 확인했으나 개선 없음.
- D8~D10: `REQUEST_MEETING` 면담 요청. 면담 진행했으나 위험도 유지.
- D11~D14: 위험도 0.91 도달 → `EMERGENCY` PENDING. 멘토 즉시 개입 필요.

**박도현 — 오탐(false alarm) 케이스**
- D1~D5: 소폭 하락. Agent `ENCOURAGE_MESSAGE` 발송.
- D6~D14: 자연 회복. 멘토 "false_alarm" 피드백 → Agent가 이 패턴을 과잉 경보로 학습.
...
- D9~D14: HIGH 5일 연속 → `REQUEST_MEETING` PENDING. 면담 대기 중.
...
구현 전에 전체 데이터 흐름과 각 계층의 책임을 먼저 문서로 정의했습니다. API 명세, DB 스키마, ML feature, Agent 행동 정책이 모두 `docs/`에 코드보다 먼저 존재하며, 이후 모든 구현의 기준이 됩니다.
...
오케스트레이터는 직접 코드를 작성하지 않습니다. 요청이 들어오면 영향 범위를 분석하고, 필요한 에이전트를 선택한 뒤, 실행 순서(`DB → Backend → AI → Frontend → Docs`)와 함께 각 에이전트에게 구체적인 프롬프트와 참고 문서를 전달합니다.
...
코드베이스 탐색에는 **code-review-graph MCP**를 활용했습니다.
